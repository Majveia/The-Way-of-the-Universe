...
// ——— Constellation figures → catalogue indices ———
function resolveStar(tok, con) {
  let c = con;
  let t = tok.trim();
  if (t.includes(':')) [t, c] = t.split(':');
  const cands = [];
  selected.forEach((s, i) => {
    if (s.con !== c) return;
    if (/^\d+$/.test(t)) {
      if (s.flam === t) cands.push(i);
    } else {
      const m = /^([A-Za-z]+)(\d)?$/.exec(t);
      if (!m) return;
      const want = m[2] ? `${m[1]}-${m[2]}` : m[1];
      if (s.bayer === want) cands.push(i);
      else if (!m[2] && s.bayer.split('-')[0] === m[1]) cands.push(-i - 1); // fallback: superscripted
    }
  });
  const exact = cands.filter((x) => x >= 0);
  if (exact.length) return exact.sort((a, b) => selected[a].mag - selected[b].mag)[0];
  const fb = cands.filter((x) => x < 0).map((x) => -x - 1);
  if (fb.length) return fb.sort((a, b) => selected[a].mag - selected[b].mag)[0];
  return -1;
}
const figOut = [];
let nSeg = 0, missing = [];
for (const [con, spec] of Object.entries(FIGURES)) {
  const segs = [];
  for (const poly of spec.split(';')) {
    const ids = poly.split('-').map((t) => {
      const i = resolveStar(t, con);
      if (i < 0) missing.push(`${t} ${con}`);
      return i;
    });
    for (let k = 0; k + 1 < ids.length; k++) {
      if (ids[k] >= 0 && ids[k + 1] >= 0 && ids[k] !== ids[k + 1]) segs.push(ids[k], ids[k + 1]);
    }
  }
  nSeg += segs.length / 2;
  figOut.push(`  ['${con}', [${segs.join(',')}]],`);
}
const figTs = `// GENERATED by src/worlds/sky/data/build/build-catalog.mjs from ./build/figures.mjs — do not edit.
// Constellation stick figures (drawn for this project) as pairs of indices into ./stars.ts.
export const CONSTELLATION_FIGURES: ReadonlyArray<readonly [string, readonly number[]]> = [
${figOut.join('\n')}
];
`;
writeFileSync(join(here, '..', 'constellations.ts'), figTs);
console.log(`constellations: ${figOut.length} figures, ${nSeg} segments; unresolved: ${missing.length ? missing.join(', ') : 'none'}`);
