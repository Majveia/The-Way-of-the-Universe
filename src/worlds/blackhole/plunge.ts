 ...
 */
import { horizonRadius, innerHorizonRadius, ksRadius, radialVelocity, rainObserverInto, type Vec3, type Vec4 } from './kerr';
...
export class PlungeTrajectory {
  ...
  t = 0;
  // Scratch (the plunge is advanced every frame: no allocations).
  private readonly u: Vec4 = [0, 0, 0, 0];
  private readonly k1: Vec4 = [0, 0, 0, 0];
  private readonly k2: Vec4 = [0, 0, 0, 0];
  private readonly k3: Vec4 = [0, 0, 0, 0];
  private readonly k4: Vec4 = [0, 0, 0, 0];
  ...
  /**
   * 4-velocity (contravariant, Kerr–Schild) of the observer now. The returned array is owned by
   * the trajectory and overwritten by the next call.
   */
  velocity(): Vec4 {
    return rainObserverInto(this.a, this.pos[0], this.pos[1], this.pos[2], this.u);
  }
  ...
  radialSpeed(): number {
    ...
    v[1] = u[2];
    v[2] = u[3];
    return radialVelocity(this.a, this.pos, v);
  }
  ...
  step(dTau: number): void {
    ...
    const p = this.pos;
    const a = this.a;
    const { k1, k2, k3, k4, v3 } = this;
    while (left > 1e-12) {
      const r = this.r;
      rainObserverInto(a, p[0], p[1], p[2], k1);
      v3[0] = k1[1];
      v3[1] = k1[2];
      v3[2] = k1[3];
      const vr = Math.abs(radialVelocity(a, p, v3, r));
      const h = Math.min(left, (0.01 * Math.max(r, 0.05)) / Math.max(vr, 1e-6));
      rainObserverInto(a, p[0] + 0.5 * h * k1[1], p[1] + 0.5 * h * k1[2], p[2] + 0.5 * h * k1[3], k2);
      rainObserverInto(a, p[0] + 0.5 * h * k2[1], p[1] + 0.5 * h * k2[2], p[2] + 0.5 * h * k2[3], k3);
      rainObserverInto(a, p[0] + h * k3[1], p[1] + h * k3[2], p[2] + h * k3[3], k4);
      for (let i = 0; i < 3; i++) p[i] += (h / 6) * (k1[i + 1] + 2 * k2[i + 1] + 2 * k3[i + 1] + k4[i + 1]);
      this.t += (h / 6) * (k1[0] + 2 * k2[0] + 2 * k3[0] + k4[0]);
      this.tau += h;
      left -= h;
    }
  }
